...
Метрики получились чуть ниже ожидаемых, возможно, дело в том, что была взята маленькая модель YOLOv8.
...
```bash
uv run python cs2-body-head-detection/train.py train
```
...
- `runs/detect/train{i}/weights/best.onnx` — ONNX лучшей модель
